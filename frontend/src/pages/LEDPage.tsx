...
export function LEDPage() {
  ...
  // Initialize ORYN LEDs when provider is dw_leds
  useEffect(() => {
  ...
  const fetchDWLedsStatus = async () => {
    try {
      ...
      console.error('Error fetching ORYN LEDs status:', error)
    }
  }
  ...
  const handleControlModeChange = async (mode: 'manual' | 'automated') => {
    ...
    try {
      ...
      toast.success(mode === 'manual' ? 'Manual / HA mode' : 'ORYN Automated mode')
    } catch {
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 text-center px-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm sm:text-base text-muted-foreground max-w-md">
            Configure your LED controller (WLED or ORYN LEDs) in the Settings page to control your lights.
          </p>
        </div>
  ...
  // Mode selector card (shared between WLED and ORYN LEDs views)
  const modeSelector = (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Choose how ORYN manages LED effects during playback and idle states
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-medium text-sm">ORYN Automated</span>
            </div>
            <p className="text-xs text-muted-foreground">
  ...
  return (
    <div className="flex flex-col w-full max-w-5xl mx-auto gap-6 py-3 sm:py-6 px-0 sm:px-4">
      ...
      <div className="space-y-0.5 sm:space-y-1 pl-1">
        ...
        <p className="text-xs text-muted-foreground">ORYN LEDs - GPIO controlled LED strip</p>
      </div>
