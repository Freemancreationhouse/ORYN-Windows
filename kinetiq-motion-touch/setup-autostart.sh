...
# Setup ORYN Touch to start automatically on boot

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ACTUAL_USER="${SUDO_USER:-$USER}"
USER_HOME=$(eval echo ~$ACTUAL_USER)

echo "=== ORYN Touch Auto-Start Setup ==="
...
setup_systemd() {
    ...
    sed "s|/home/pi/oryn-touch|$SCRIPT_DIR|g" "$SCRIPT_DIR/oryn-touch.service" > /tmp/oryn-touch.service
    ...
    echo "   sudo systemctl stop oryn-touch" 
    echo "   sudo systemctl status oryn-touch"
    echo "   sudo journalctl -u oryn-touch -f"
}
...
setup_desktop() {
    ...
    sed "s|/home/pi/oryn-touch|$SCRIPT_DIR|g" "$SCRIPT_DIR/oryn-touch.desktop" > "$USER_HOME/.config/autostart/oryn-touch.desktop"
    
    # Make sure the user owns the file
    chown $ACTUAL_USER:$ACTUAL_USER "$USER_HOME/.config/autostart/oryn-touch.desktop"
