...
while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
    if curl -s --connect-timeout 2 "$BACKEND_URL/serial_status" > /dev/null 2>&1; then
        ...
        if [ $ATTEMPT -eq 1 ]; then
            ...
            echo "   Make sure the main ORYN application is running"
...
if [ $ATTEMPT -eq $MAX_ATTEMPTS ]; then
    ...
    echo "   Please ensure the main ORYN application is running at localhost:8080"
...
echo ""
echo "🚀 Starting ORYN Touch (development mode)"
