# ORYN Touch Interface

A PySide6/QML touch interface for the ORYN sand table system that works alongside the existing FastAPI web server.
...
```bash
# Control the service
sudo systemctl start oryn-touch    # Start now
sudo systemctl stop oryn-touch     # Stop service
sudo systemctl status oryn-touch   # Check status
sudo journalctl -u oryn-touch -f   # View logs

# Disable auto-start
sudo systemctl disable oryn-touch
```
...
2. Ensure the main ORYN FastAPI server is running:
   ```bash
   cd ../  # Go to main oryn directory
   python main.py
   ```
...
- Paths are relative to the main oryn directory