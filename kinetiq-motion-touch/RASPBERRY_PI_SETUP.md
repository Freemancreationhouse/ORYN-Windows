# Raspberry Pi MIPI DSI Touchscreen Setup for ORYN Touch
...
```bash
# Start the service
sudo systemctl start oryn-kiosk

# Stop the service
sudo systemctl stop oryn-kiosk

# Check service status
sudo systemctl status oryn-kiosk

# View live logs
journalctl -u oryn-kiosk -f

# Disable auto-start
sudo systemctl disable oryn-kiosk

# Re-enable auto-start
sudo systemctl enable oryn-kiosk
```
...
```bash
sudo tee /etc/systemd/system/oryn-touch.service << EOF
[Unit]
...
WorkingDirectory=/home/pi/oryn/oryn-touch
ExecStart=/home/pi/oryn/oryn-touch/bin/python main.py
Restart=always
RestartSec=10
...
# Enable the service
sudo systemctl enable oryn-touch.service
sudo systemctl start oryn-touch.service
```
