#!/bin/bash
#
# ORYN - Autohotspot Setup
#
# Sets up the autohotspot system so the Pi creates a "ORYN" WiFi
# hotspot when no known network is available. Users connect to the hotspot
# and configure WiFi through the ORYN app.
#
...
install_dns_redirect() {
    ...
    sudo cp "$SCRIPT_DIR/dnsmasq-hotspot.conf" "$NM_DNSMASQ_DIR/oryn-captive.conf"

    print_success "DNS redirect installed at $NM_DNSMASQ_DIR/oryn-captive.conf"
}
...
# Main
main() {
    echo -e "${GREEN}ORYN Autohotspot Setup${NC}"
    echo ""
    ...
    echo ""
    echo "Users can connect to the hotspot and open the ORYN app"
    echo "to configure WiFi credentials."
    echo ""
    echo "Commands:"
    echo "  oryn wifi status   — Show current WiFi mode"
    echo "  oryn wifi hotspot  — Manually switch to hotspot mode"
    echo "  oryn wifi scan     — Scan for available networks"
}
