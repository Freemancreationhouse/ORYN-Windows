...
def update_software():
    ...
    For a full update (including frontend rebuild), run 'kinetiq update' instead.
    """
    error_log = []
    ...
    logger.info("Restarting kinetiq-motion service...")
    restart_result = run_command(
        [SUDO, SYSTEMCTL, "restart", "kinetiq-motion"],
        "Failed to restart kinetiq-motion service"
    )

    if not restart_result:
        error_log.append("Service restart failed - please run 'kinetiq restart' manually")

    if error_log:
        logger.error(f"Software update completed with errors: {error_log}")
        return False, "Update completed with errors. Run 'kinetiq update' for a full update.", error_log
