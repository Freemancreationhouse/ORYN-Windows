...
# Try to import KinetiQ LED controller - it requires RPi-specific dependencies
try:
    ...
    # Running on non-RPi platform - KinetiQ LEDs not available
    ...
    def __init__(self, provider: LEDProviderType = "none", ip_address: Optional[str] = None,
        ...
        if provider == "wled" and ip_address:
            ...
            if not DW_LEDS_AVAILABLE:
                raise ImportError("KinetiQ LED controller requires Raspberry Pi GPIO libraries. Install with: pip install -r requirements.txt")
            # KinetiQ LEDs uses local GPIO, no IP needed
