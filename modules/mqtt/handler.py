...
class MQTTHandler(BaseMQTTHandler):
    ...
    def __init__(self, callback_registry: Dict[str, Callable]):
        ...
        self.status_topic = os.getenv('MQTT_STATUS_TOPIC', 'kinetiq_motion/status')
        self.command_topic = os.getenv('MQTT_COMMAND_TOPIC', 'kinetiq_motion/command')
        ...
        self.device_name = state.mqtt_device_name if state.mqtt_device_name else os.getenv('HA_DEVICE_NAME', 'KinetiQ Motion')
        self.device_id = state.mqtt_device_id if state.mqtt_device_id else os.getenv('HA_DEVICE_ID', 'kinetiq_motion')
    ...
    def setup_ha_discovery(self):
        ...
        # LED Control Entities (only for KinetiQ LEDs - WLED has its own MQTT integration)
    ...
    def _publish_led_state(self):
        """Helper to publish LED state to MQTT (KinetiQ LEDs only - WLED has its own MQTT)."""
        ...
        try:
            ...
            # Publish power state (check both "power" for WLED compatibility and "power_on" for KinetiQ LEDs)
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:
            if msg.topic == self.pattern_select_topic:
                ...
                # Handle LED power command (KinetiQ LEDs only)
                payload = msg.payload.decode()
                ...
                # Handle LED brightness command (KinetiQ LEDs only)
                ...
                if 0 <= brightness <= 100 and state.led_controller and state.led_provider == "dw_leds":
                    ...
                    if controller and hasattr(controller, 'set_brightness'):
                        # KinetiQ LED controller expects 0-100, converts internally to 0.0-1.0
                        controller.set_brightness(brightness)
                        self.client.publish(f"{self.device_id}/led/brightness/state", brightness, retain=True)
            elif msg.topic == self.led_effect_topic:
                # Handle LED effect command (KinetiQ LEDs only)
                ...
                # Handle LED speed command (KinetiQ LEDs only)
                speed = int(msg.payload.decode())
                ...
                # Handle LED intensity command (KinetiQ LEDs only)
                ...
                # Handle LED color command (RGB) (KinetiQ LEDs only)
                try:
